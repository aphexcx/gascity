...
func ComputeAwakeSet(input AwakeInput) map[string]AwakeDecision {
	...
	for template, count := range input.ScaleCheckCounts {
		...
		active := input.excludeDeferredSessions(collectActiveBeads(input.SessionBeads, template))
	...
	for template, hasWork := range input.WorkSet {
		...
		if active := input.excludeDeferredSessions(collectActiveBeads(input.SessionBeads, template)); len(active) > 0 {
