...
func TestReconcileSessionBeads_FailedCreatePoolSlotIsReplacedUnderTheSameRuntimeName(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			cfgNames := configuredSessionNames(cfg, cfg.EffectiveCityName(), store)
			poolDesired := PoolDesiredCounts(ComputePoolDesiredStates(cfg, firstTick.AssignedWorkBeads, firstTick.AssignedWorkStoreRefs, sessionInfosFromBeads(sessions), firstTick.ScaleCheckCounts))
			if poolDesired == nil {
				poolDesired = make(map[string]int)
			}
