...
func TestAutocloseOwningStoreAnnouncesAFaultOnce(t *testing.T) {
	...
	for range 2 {
		if store, _, outcome := autocloseOwningStore("hq-1", cityPath, cityPath, sink); outcome == autocloseResolved {
...
func TestAutocloseOwningStoreStaysQuietOnAbsence(t *testing.T) {
	...
	if store, _, outcome := autocloseOwningStore("hq-nothing-here", cityPath, cityPath, sink); outcome == autocloseResolved {
...
// A migrated binding wins over a retained city copy, but a rig that cannot
// open has not proved the legacy work-prefixed ID is unique. The strict
// mutation resolver must refuse before giving close/land a writable store.
func TestConvoyBindingOwnerRequiresReadableRig(t *testing.T) {
	for _, tc := range []struct {
		name                                                     string
		rigFails, rigReadFails, cityFails, reserved, wantRefusal bool
	}{
		{name: "unreadable rig with readable city", rigFails: true, wantRefusal: true},
		{name: "rig opens but canonical read fails", rigReadFails: true, wantRefusal: true},
		{name: "unreadable rig and city", rigFails: true, cityFails: true, wantRefusal: true},
		{name: "unreadable retained city does not veto binding", cityFails: true},
		{name: "reserved binding id needs no rig probe", rigFails: true, reserved: true},
	} {
		t.Run(tc.name, func(t *testing.T) {
			...
			resident, err := migrationSeed(binding, beads.Bead{ID: id, Title: "binding convoy", Type: "convoy"})
			if err != nil {
				t.Fatal(err)
			}
			city := splittest.NewWorkStore(t, "hq")
			rig := splittest.NewWorkStore(t, "hq")
			unavailable := errors.New("rig unavailable during ownership check")
			if tc.rigReadFails {
				unavailable = errConvoyStoreDark
			}
			openStore := func(dir string) (beads.Store, error) {
				if samePath(dir, rigPath) {
					...
					if tc.rigReadFails {
						return newConvoyDarkStore(), nil
					}
					return rig, nil
				}
				if samePath(dir, cityPath) {
					if tc.cityFails {
						return nil, errors.New("retained city unavailable")
					}
					return city, nil
				}
				t.Fatalf("unexpected store %s", dir)
				return nil, nil
			}
			store, _, err := resolveConvoyStoreForCommand(resident.ID, cfg, cityPath, convoyMutationStrict, openStore)
			if tc.wantRefusal {
				if err == nil || store != nil {
					t.Errorf("strict binding resolution returned store=%v err=%v despite an unreadable rig", store, err)
				}
				if err != nil && (!errors.Is(err, unavailable) || !strings.Contains(err.Error(), rigPath)) {
					t.Fatalf("ownership refusal must name rig and preserve cause: %v", err)
				}
			} else if err != nil || store != binding {
				t.Fatalf("binding resolution = (%v,%v), want admitted binding", store, err)
			}
			for _, source := range []string{cityPath, rigPath} {
				autostore, _, outcome, skipped, autoerr := resolveAutocloseOwner(resident.ID, cfg, cityPath, source, openStore)
				if tc.wantRefusal {
					if outcome != autocloseVetoed || autostore != nil || len(skipped) == 0 || !errors.Is(autoerr, unavailable) {
						t.Errorf("autoclose source %s = (store=%v, outcome=%v, skipped=%v, err=%v), want ownership veto with unavailable rig", source, autostore, outcome, skipped, autoerr)
					}
				} else if outcome != autocloseResolved || autostore != binding || autoerr != nil {
					t.Errorf("autoclose source %s = (%v,%v,%v), want resolved binding", source, autostore, outcome, autoerr)
				}
			}
		})
	}
