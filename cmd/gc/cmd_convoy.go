...
func openConvoyStoresWithSkipped(cfg *config.City, cityPath, beadID string, openStore func(string) (beads.Store, error)) ([]convoyStoreView, []storeProbeFailure, error) {
	...
	if len(skipped) > 0 {
		return nil, skipped, skipped[0].err
	}
...
func resolveOwningStoreDirWithSkipped(beadID string, cfg *config.City, cityPath string, openStore func(string) (beads.Store, error)) (beads.Store, string, []storeProbeFailure, error) {
	...
	if ownedByBinding {
		skipped, err := bindingRigCollisionWithSkipped(beadID, cfg, cityPath, openStore)
		if err != nil {
			return nil, "", skipped, err
		}
		return owner.Store, cityPath, nil, nil
	}
...
func refuseBindingRigCollision(beadID string, cfg *config.City, cityPath string, openStore func(string) (beads.Store, error)) error {
	_, err := bindingRigCollisionWithSkipped(beadID, cfg, cityPath, openStore)
	return err
}

// bindingRigCollisionWithSkipped preserves unreadable rig evidence so autoclose
// can veto instead of treating a refused binding lookup as permission to fall
// back to the event's source store. The retained city copy is not a competitor.
func bindingRigCollisionWithSkipped(beadID string, cfg *config.City, cityPath string, openStore func(string) (beads.Store, error)) ([]storeProbeFailure, error) {
	...
	rigSkipped := make([]storeProbeFailure, 0, len(skipped))
	for _, failure := range skipped {
		if !samePath(failure.path, cityPath) {
			rigSkipped = append(rigSkipped, failure)
		}
	}
	for _, candidate := range candidates {
		...
		if _, err := candidate.store.Get(beadID); err != nil {
			...
			rigSkipped = append(rigSkipped, storeProbeFailure{path: candidate.path, err: err})
			continue
		}
		return rigSkipped, fmt.Errorf("bead %s exists in multiple stores (%s and %s); resolution requires a uniquely addressable bead id", beadID, convoyBindingViewPath, candidate.path)
	}
	if len(rigSkipped) > 0 {
		return rigSkipped, partialVisibilityError(beadID, rigSkipped)
	}
	return nil, nil
}
...
// resolveOwningStoreDir is the strict by-id ownership surface. Degraded read
// commands use resolveOwningStoreDirWithSkipped and announce skipped stores.
