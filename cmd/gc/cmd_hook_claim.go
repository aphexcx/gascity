...
type hookClaimOps struct {
	...
	// establish the durable lifecycle-start emission point and cross-city write fence.
...
// installCrossCityWriteFence wraps the hook's claim-time write seams —
// Claim (the fresh-claim and ready-promotion CAS), ReclaimStale, and
// AssignContinuation (the sibling preassign) — with an authoritative re-check of the bead about to be
...
func (ops *hookClaimOps) installCrossCityWriteFence(opts hookClaimOptions, stderr io.Writer) {
	...
	ops.ReclaimStale = func(ctx context.Context, dir string, env []string, beadID string) (bool, string, error) {
		refused, err := hookCrossCityRefusedAtWrite(ctx, dir, env, beadID, opts, base, "reclaim", stderr)
		if err != nil {
			return false, "", fmt.Errorf("cross-city fence: %s could not be verified before reclaim, nothing written: %w", beadID, err)
		}
		if refused {
			return false, "", errCrossCityFenced
		}
...
func hookClaimIdentityPatch(bead beads.Bead, opts hookClaimOptions, ops hookClaimOps, dir string) map[string]string {
	...
	// The fork exports an explicit agent work_dir as GC_DIR. Preserve that
	// branch evidence when the bead and durable session have no checkout, but
	// pass it through the same store exclusion and do not mint worktree ownership.
	if len(trees) == 0 && hookClaimRecordsNoWorkDir(bead) && sessionID != "" && !isControl &&
		hookClaimWorktreeEvidenceIsWholeOrAbsent(bead) {
		if tree, admitted := storeHead.Admit(hookClaimWorkBranchDir(opts.Env, dir)); admitted {
			trees = []hookClaimWorkTree{tree}
		}
	}
