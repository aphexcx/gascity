...
func TestPoolStartBackoff_ParksAfterMaxFailuresWithOneMailAndNoStart(t *testing.T) {
	...
	if rows := poolDemandAssignedWork(h.cfg, "", nil, nil, []beads.Bead{work}, []string{""}, pass); len(rows) != 0 {
...
func TestPoolStartBackoff_DeadlineJudgedByTheClock(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		rows := poolDemandAssignedWork(h.cfg, "", nil, nil, []beads.Bead{work}, []string{""}, pass)
