...
// which case every mail nudge would be withdrawn as "mail-gone" instead of
// delivered. Two real beadmail messages, one read; the unread one must reach
// the agent and the read one must be withdrawn as "mail-read".
func TestCmdNudgeDrainReValidatesMailAgainstRealProvider(t *testing.T) {
	...
	if withdrawn.TerminalReason != "mail-read" {
		t.Fatalf("terminal_reason = %q, want mail-read", withdrawn.TerminalReason)
	}
}
...
func TestSplitQueuedNudgesForDelivery_BlocksCanceledWaitNudge(t *testing.T) {
	...
	deliverable, blocked, _, err := splitQueuedNudgesForDelivery(queuedNudgeDeliveryGate{sessFront: sessionFrontDoor(store)}, []queuedNudge{{
		ID:        "n1",
		Agent:     "worker",
		Source:    "wait",
...
// message read in the meantime is withdrawn as "mail-read"; a
// message that has vanished (archived — #4422 deletes the bead) is
// withdrawn as "mail-gone" rather than surfaced as an error.
func TestBlockedQueuedMailNudgeReason_ReReadsMessageAtDelivery(t *testing.T) {
	...
	cases := []struct {
		...
		{"already-read-withdrawn", mp, mailItem(readMsg.ID), "mail-read", true},
		{"missing-withdrawn", mp, mailItem("gc-nope"), "mail-gone", true},
		{"archived-withdrawn-as-missing", mp, mailItem(archived.ID), "mail-gone", true},
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			reason, block, err := blockedQueuedNudgeReason(testMailProviderDeliveryGate(tc.mp), tc.item)
			if err != nil {
				t.Fatalf("blockedQueuedNudgeReason: %v", err)
			}
			if reason != tc.wantReason || block != tc.wantBlock {
...
func TestSplitQueuedNudgesForDelivery_MailNudges(t *testing.T) {
	...
	deliverable, blocked, _, err := splitQueuedNudgesForDelivery(testMailProviderDeliveryGate(mp), items)
	if err != nil {
		t.Fatalf("splitQueuedNudgesForDelivery: %v", err)
	}
	...
	if got := blocked["mail-read"]; len(got) != 1 || got[0].ID != "n-read" {
		t.Fatalf("blocked[mail-read] = %#v, want n-read", blocked["mail-read"])
	}
	if got := blocked["mail-gone"]; len(got) != 1 || got[0].ID != "n-missing" {
		t.Fatalf("blocked[mail-gone] = %#v, want n-missing", blocked["mail-gone"])
	}
}
...
func testMailProviderDeliveryGate(mp mail.Provider) queuedNudgeDeliveryGate {
	gate := queuedNudgeDeliveryGate{}
	if mp != nil {
		gate.mailState = func(id string) (mailReminderState, error) {
			msg, err := mp.Get(id)
			if errors.Is(err, mail.ErrNotFound) {
				return mailReminderGone, nil
			}
			if err != nil {
				return mailReminderUnread, err
			}
			if msg.Read {
