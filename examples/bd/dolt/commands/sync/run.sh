...
sync_database_sql() {
  ...
  if [ "$force" != true ]; then
    ...
    if [ "$inflight_rc" -ne 0 ]; then
      ...
      last_fail_reason="processlist query failed (exit $inflight_rc)"
    ...
    if bound_expired "$fetch_rc"; then
      ...
      rm -f "$fetch_err_tmp"
      last_fail_reason="server refused a second fetch"
      ...
      # the client.
      rm -f "$fetch_err_tmp"
      last_fail_reason="fetch session lost the run lock"
