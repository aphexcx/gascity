...
// runFFSyncEnv retains upstream remote-selection fixtures while sharing the
// fork's single command construction and isolated TCP listener ownership.
...
func runFFSync(t *testing.T, binDir string, args ...string) string {
	t.Helper()
	out, _ := ffSyncCmd(t, binDir, nil, args...).CombinedOutput()
	return string(out)
}
