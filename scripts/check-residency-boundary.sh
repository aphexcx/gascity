...
# scripts/residency_signature_rule_test.go and scripts/residency_expression_rules_test.go), which shares this baseline file so there is
...
while IFS=$'\t' read -r kind path fn pattern have want; do
	...
	GROW)
		...
		;;
	SHRINK)
		printf 'RESIDENCY-BOUNDARY: %s %s %s: %d sites, baseline %d — the baseline must SHRINK in the same commit that retires a site. Follow the regeneration procedure in the header of scripts/residency-boundary-baseline.txt; do NOT redirect --emit-baseline over that file.\n' "$path" "$fn" "$pattern" "$have" "$want"
		;;
	MALFORMED)
