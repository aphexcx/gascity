...
`
	...
	p.c.bin = script
	// Keep the fake listener out of the user's herdr config directory on every
	// platform. A short root also stays within Darwin's Unix socket path limit.
	socketDir, err := os.MkdirTemp("/tmp", "herdr-fake-")
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() { _ = os.RemoveAll(socketDir) })
	p.c.sockPath = filepath.Join(socketDir, "herdr.sock")
