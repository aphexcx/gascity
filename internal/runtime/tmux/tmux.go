...
// sendStartupTextWithRetry preserves byte evidence across provider-safe chunks.
// A later failure retains the bytes already pasted and the partial-paste error
// that requires startup callers to discard the incomplete session.
func (t *Tmux) sendStartupTextWithRetry(target, text, provider string, timeout time.Duration) (int, error) {
	if len(text) > copilotMaxPasteBytes && sessionlog.ProviderFamily(provider) == "copilot" {
		...
		written := 0
		err := sendPasteChunks(chunks, func(chunk string) error {
			remaining := time.Until(deadline)
			if remaining <= 0 {
				return fmt.Errorf("agent not ready for input after %s", timeout)
			}
			n, err := t.sendTextWithRetry(target, chunk, remaining, t.pasteLiteralText)
			written += n
			return err
		}, func() {
			remaining := time.Until(deadline)
...
func (t *Tmux) sendTextWithRetry(target, text string, timeout time.Duration, send func(string, string) (int, error)) (int, error) {
	deadline := time.Now().Add(timeout)
	interval := t.cfg.NudgeRetryInterval
	var lastErr error

	for time.Now().Before(deadline) {
		written, err := send(target, text)
		if err == nil {
			return written, nil
		}
...
//   - Submit is confirmed when the pane went busy or its composer drained,
//     unconfirmed when a busy
...
//
// A send failure before anything landed has zero evidence. A partial startup
// paste retains its byte count alongside the error. Drained-composer acceptance
// retains the upstream diagnostic alongside confirmed evidence; the provider
// adapter consumes that diagnostic without reporting a delivery failure.
func (t *Tmux) nudgeSessionDelivery(session, message string) (runtime.NudgeDelivery, error) {
	return t.nudgeSession(session, message, t.sendKeysLiteralWithRetry,
		t.shouldSendEscapeBeforeEnter, t.nudgeSubmitKeySequence)
}

// nudgeStartupSession uses provider-safe chunks while preserving the ordinary
// submit retry contract and treating a partial role prompt as a startup failure.
func (t *Tmux) nudgeStartupSession(session, message string) error {
	delivery, err := t.nudgeSession(session, message,
		func(target, text string, timeout time.Duration) (int, error) {
			return t.sendStartupTextWithRetry(target, text, t.providerEnv(target), timeout)
		}, t.shouldSendEscapeBeforeEnter, t.nudgeSubmitKeySequence)
	if err != nil {
		return err
	}
...
func (t *Tmux) nudgeSession(
	...
	sendText func(string, string, time.Duration) (int, error),
	shouldSendEscape func(string) bool,
	submitKeySequence func(string) []string,
) (runtime.NudgeDelivery, error) {
	...
	// 2. Paste the payload, preserving evidence if a later startup chunk fails.
	written, err := sendText(target, message, t.cfg.NudgeReadyTimeout)
	if err != nil {
		if written > 0 {
			delivered = true
			payloadBytes = written
			return runtime.NudgeDelivery{Delivered: true, Bytes: written, Submit: runtime.NudgeSubmitUnconfirmed}, err
		}
		return runtime.NudgeDelivery{}, err
	}
	payloadBytes = written
	...
	if t.submitVerifyEligible(target) {
		...
		if !confirmed {
			// A drained composer proves acceptance even when the busy indicator
			// was missed. Preserve the upstream diagnostic for error-only callers;
			// evidence-aware callers receive the confirmed delivery alongside it.
			if lines, capErr := t.CapturePaneLines(target, promptObservationLines); capErr == nil && paneShowsDrainedComposer(lines, message) {
				submitted = true
				return runtime.NudgeDelivery{Delivered: true, Bytes: payloadBytes, Submit: runtime.NudgeSubmitConfirmed}, fmt.Errorf("%w: session %q", ErrNudgeSubmitDeliveredUnobserved, session)
			}
		}
		submit := runtime.NudgeSubmitUnconfirmed
		if confirmed {
