...
func TestEnsureRunning_EmptySessionKeyStripsResumeAndStartsFresh(t *testing.T) {
	...
	delivery, err := mgr.Send(context.Background(), info.ID, "hello", resumeCmd, runtime.Config{WorkDir: "/tmp"})
	if err != nil {
		t.Fatalf("Send should recover with a fresh start when session_key is empty, got: %v", err)
	}
	if want := (runtime.NudgeDelivery{Delivered: true}); delivery != want {
		t.Fatalf("Send delivery = %+v, want %+v after fresh recovery", delivery, want)
	}
...
func TestEnsureRunning_EmptySessionKeyWithoutResumeShapeDoesNotRelaunch(t *testing.T) {
	...
	delivery, err := mgr.Send(context.Background(), info.ID, "hello", freshCmd, runtime.Config{WorkDir: "/tmp"})
	if err == nil {
		t.Fatal("Send should fail when the start dies and there is no resume shape to strip")
	}
	if delivery != (runtime.NudgeDelivery{}) {
		t.Fatalf("Send delivery = %+v, want no evidence after startup failure", delivery)
	}
...
func TestEnsureRunning_EmptySessionKeyStripsDivergedSessionIDAndStartsFresh(t *testing.T) {
	...
	delivery, err := mgr.Send(context.Background(), info.ID, "hello", firstStartCmd, runtime.Config{WorkDir: "/tmp"})
	if err != nil {
