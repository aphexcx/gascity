...
| SESSION-WORK-005 | Pool claim survives a demand census gap | A running pool session stays open when live work matches its canonical lane identity or claim-time session ID. Work bound to another session and reusable slot aliases do not protect it. Pool sessions with a trigger or current claim require two consecutive absence confirmations before draining; recovered demand or failed observations reset the window. Current-claim reads and in-progress demand reads observe external writes before cache reconciliation. | gf-2uk; `cmd/gc/session_reconciler_pool_census_gap_test.go`; `cmd/gc/session_alias_assignment_guard_test.go`; `internal/session/store_test.go` |
...
| SESSION-RUNTIME-010 | Pre-start orphan cleanup preserves shared infrastructure | Process-table scans exclude infrastructure processes even if they inherited a session ID. Fresh tmux servers do not inherit the caller's session identity; explicitly supplied pane identity remains intact. Each successful pre-start orphan termination logs session ID, PID, and provider name. | `internal/runtime/proctable/scan_darwin_test.go`; `internal/runtime/proctable/scan_linux_test.go`; `internal/runtime/tmux/server_identity_integration_test.go`; `internal/session/manager_test.go#TestCreateKillsUntrackedOrphanBeforeStart`; gp-ry5d |
