...
## print-cgo-env: print Bash assignments for exported CGO flags without building
print-cgo-env:
	@bash -eu -c 'for cgo_name in CGO_CPPFLAGS CGO_LDFLAGS; do \
		if declare -p "$$cgo_name" >/dev/null 2>&1; then \
			printf "%s=%q\n" "$$cgo_name" "$${!cgo_name}"; \
		fi; \
	done'
